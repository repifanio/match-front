...
export default function App() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Limpa o JSON do output para não poluir o Markdown
      ...
      <main className="max-w-[1400px] mx-auto p-10">
        ...
        <div className="grid grid-cols-12 gap-10 items-start">
          ...
          <div className="col-span-12 lg:col-span-4 space-y-8">
            <div className="bg-white border border-indigo-50 rounded-3xl p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
              ...
              <div className="space-y-4">
                <label className="text-xs font-bold text-slate-700 ml-1">Descrição da Vaga</label>
                <textarea
                  rows={10}
              ...
              {/* Card de Suporte Anônimo e Elegante */}
              <div className="mt-8 pt-8 border-t border-indigo-50/50">
                <div className="bg-slate-50/50 rounded-2xl p-5 border border-slate-100/50">
                  ...
                  <a
                    href="mailto:suporte@matchpro.ai"
                    className="flex items-center justify-center gap-2 w-full py-2.5 bg-white border border-slate-200 hover:border-indigo-200 hover:bg-indigo-50 text-slate-600 hover:text-indigo-600 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-sm"
                  >
                    Enviar E-mail
                  </a>
                </div>
          ...
          <div className="col-span-12 lg:col-span-8 bg-white border border-indigo-50 rounded-[2.5rem] min-h-[750px] flex flex-col shadow-[0_20px_50px_rgba(0,0,0,0.03)] overflow-hidden relative">
            ...
            {/* Loader Overlay */}
            {loading && (
              <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-white/70 backdrop-blur-md animate-in fade-in duration-500">
                <div className="relative mb-8">
                  <div className="absolute inset-0 bg-blue-400 blur-3xl opacity-20 animate-pulse"></div>
                  <div className="relative bg-white p-6 rounded-[2rem] shadow-2xl border border-indigo-50 animate-bounce">
            ...
            <div className="p-8 border-b border-indigo-50 bg-gradient-to-r from-slate-50/50 to-white flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs font-black uppercase tracking-[0.3em] text-slate-400 font-bold">Relatório Executivo</span>
              </div>
              {analysis && (
                <button
                  onClick={() => {
              ...
              {!analysis ? (
                ...
                <>
                  <article className="prose prose-indigo max-w-none 
                    ...
                    prose-li:text-slate-600 animate-in fade-in slide-in-from-bottom-6 duration-700">
                    <ReactMarkdown remarkPlugins={[remarkGfm]}>
                      {analysis}
                    </ReactMarkdown>
                    {/* SEÇÃO PIX - DESIGN FIXO E ACESSÍVEL */}
                    <div className="mt-20 p-10 bg-slate-50 rounded-[2.5rem] border border-slate-100 text-center animate-in zoom-in duration-700">
                      <div className="inline-flex items-center justify-center w-12 h-12 bg-white rounded-2xl shadow-sm mb-6">
                        <Zap className="w-6 h-6 text-amber-400 fill-amber-400" />
                      </div>

                      <h3 className="text-2xl font-black text-slate-800 tracking-tight">O Match Pro te ajudou?</h3>
                      <p className="text-slate-500 text-sm mt-2 max-w-sm mx-auto leading-relaxed font-medium">
                        Este projeto é mantido por doações. Se esta análise agregou valor, considere apoiar para mantermos a IA ativa.
                      </p>

                      <div className="mt-10 flex flex-col items-center">
                        {/* QR CODE ABERTO E GRANDE */}
                        <div className="bg-white p-6 rounded-[2rem] shadow-xl border border-indigo-50 mb-8">
                          ...
                          <div className="mt-4 pt-4 border-t border-slate-50">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
                              Rafael Camargo Epifanio
                            </p>
                          </div>
                        </div>

                        {/* BOTÃO COPIA E COLA LOGO ABAIXO */}
                        <button
                          onClick={() => {
                        ...
                        >
                          ...
                          {pixCopied ? 'Chave Copiada!' : 'PIX Copia e Cola'}
                        </button>
                      </div>

                      <p className="mt-10 text-[10px] font-bold text-slate-300 uppercase tracking-[0.3em]">
                        Obrigado por fortalecer a comunidade dev
                      </p>
                    </div>
                  </article>
                </>
